...
def CyclicScore(peptide, experimental_spectrum):
    theoretical_spectrum = CyclicSpectrum(Peptide=peptide, Alphabet=Alphabet, AminoAcidMass=aminoAcidMass)
    score = 0
    exp_spectrum_copy = experimental_spectrum.copy()
    
    for sp in theoretical_spectrum:
        if sp in exp_spectrum_copy:
            exp_spectrum_copy.remove(sp)
            score += 1
    ...
    while Leaderboard:
        ...
        for peptide in list(Leaderboard)[:]:
            if Mass(peptide, AminoAcidMass) == ParentMass(Spectrum):
                if CyclicScore(peptide, Spectrum) > CyclicScore(LeaderPeptide, Spectrum):
                    LeaderPeptide = peptide
            elif Mass(peptide, AminoAcidMass) > ParentMass(Spectrum):
                Leaderboard.remove(peptide)
        Leaderboard = Trim(Leaderboard, Spectrum, N)
    LeaderPeptideMass = [Mass(p, aminoAcidMass) for p in LeaderPeptide]
    print(len(list(Leaderboard)))
    return "-".join(map(str, LeaderPeptideMass))

# leaderboard = ""
experimental_spectrum = []
N = 0
with open("3.exp.txt") as file:
...
# )
final = LeaderboardCyclopeptideSequencing(
    Spectrum=experimental_spectrum, 
    N=N, 
    Alphabet=Alphabet, 
    AminoAcidMass=aminoAcidMass
